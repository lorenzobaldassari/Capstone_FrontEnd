...
const PostProfilePagina = ({ post, id }) => {
  ...
  const deletePost = (id) => {
    fetch("http://localhost:3010/posts/" + id + "/me", {
      headers: {
  ...
  return (
    <>
      ...
      <Row className="my-4 g-3 mx-2">
        <h1>PROFILO PAGINA</h1>
        {post.map((elem) => {
          return (
            <div key={elem.uuid}>
              <Col xs={12} className="position-relative">
                {elem.paginaPost.id === uuid && (
                  ...
                  <div className="d-flex align-items-center justify content-start">
                    ...
                    <div>
                      <Link to={"/profiloPagina/" + elem.paginaPost.id}>
                        <h6 className="mb-0 ms-3">{elem.paginaPost.titolo}</h6>
                      </Link>
...
export default PostProfilePagina;
